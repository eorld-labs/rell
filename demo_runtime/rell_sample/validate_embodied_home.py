...
def main() -> None:
    ...
    require(approach_sofa["active_role"] == "spatial_target" and approach_sofa["status"] == "fact_established", f"sofa did not bind as an executable navigation target: {approach_sofa}")
    ...
    require(approach_started["frame_count"] > 0, f"object-relative motion did not preserve a stable frame plan: {approach_started}")
    approach_completed = drain_motion(approach_started)
    approach_result = approach_completed["result"]
    require(approach_result["status"] == "fact_established" and approach_result["terminal_fact"] == "executor_near_object", f"object-relative terminal fact was not established after execution: {approach_completed}")
    require(approach_result["terminal_verification_evidence"]["relation_verified"], f"last frame did not verify object-relative relation: {approach_completed}")
    role_session = start_session()
    role_session_id = role_session["session_id"]
    ...
    multiple_perception = multiple["task_perception"]
    require(multiple["status"] == "process_grounding_clarification_required" and multiple.get("pending_slot") == "theme", f"multiple cups did not enter the unified theme gap: {multiple}")
    require(multiple_perception["concept_grounding"]["ambiguity_reason"] == "multiple_target_candidates", f"wrong ambiguity reason: {multiple}")
    require(multiple_perception["concept_grounding"]["candidate_summary"]["target_count"] == 2, f"both cups were not retained: {multiple}")
    require(len((multiple.get("grounding_gap") or {}).get("candidate_values", [])) == 2, f"slot candidates missing from normalized gap: {multiple}")
    require(not multiple_perception["concept_grounding"]["candidate_bindings"], f"ambiguous perception selected a cup: {multiple}")
    require(not multiple_perception["concept_grounding"]["runtime_fact_committed"], f"ambiguous candidates became fact: {multiple}")
