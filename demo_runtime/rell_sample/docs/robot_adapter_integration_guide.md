...
2. Adapter 能对 `align` 阶段输出 `spout_to_cup_distance`。
3. Adapter 能对 `tilting` 阶段输出 `tilt_angle` 和 `water_flow_rate`。
4. Adapter 能对 `maintain_flow` 阶段输出至少两个 `cup_has_water` 观测通道。
5. Runtime 能在成功路径输出 `completed`。
6. Runtime 能在失败或冲突路径输出 `requires_human_confirmation`。
