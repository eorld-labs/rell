...
def main() -> None:
    ...
    long_intent_session = start_session(scene_id="home_semantic_3d_b", executor_profile_id="home_humanoid")
    long_intent_first = begin_motion_command(long_intent_session["session_id"], "把杯子放到餐桌上")
    first_intent = long_intent_first["immediate_result"]["long_horizon_intent"]
    require(long_intent_first["status"] == "requires_human_confirmation" and first_intent["goal_fact"] == "object_supported_at_destination", f"long goal did not create a root intent contract: {long_intent_first}")
    require(long_intent_first["immediate_result"]["long_stage"]["stage_id"] == "acquire_theme", f"long goal did not derive the grasp stage from current facts: {long_intent_first}")
    long_intent_paused = begin_motion_command(long_intent_session["session_id"], "暂停当前任务")
    require(long_intent_paused["status"] == "long_intent_suspended" and long_intent_paused["immediate_result"]["long_horizon_intent"]["resume_envelope"]["old_path_discarded"], f"long intent suspension retained a stale path: {long_intent_paused}")
    long_intent_resumed = begin_motion_command(long_intent_session["session_id"], "继续当前任务")
    require(long_intent_resumed["status"] == "requires_human_confirmation" and long_intent_resumed["immediate_result"]["long_stage"]["stage_id"] == "acquire_theme", f"long intent resume did not rederive the current stage: {long_intent_resumed}")
    long_intent_started = begin_motion_command(long_intent_session["session_id"], "确认")
    long_intent_grasped = drain_motion(long_intent_started)
    require(long_intent_grasped["result"]["terminal_fact"] == "target_object_in_gripper", f"long intent first stage did not verify grasp: {long_intent_grasped}")
    long_intent_place_pending = long_intent_grasped["result"]["pending_confirmation"]
    require(long_intent_place_pending["long_stage_id"] == "place_at_destination", f"long intent did not rebuild the placement stage after verified grasp: {long_intent_grasped}")
    long_intent_place_started = begin_motion_command(long_intent_session["session_id"], "确认")
