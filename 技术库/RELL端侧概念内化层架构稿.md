...
第一阶段的最小澄清闭环优先采用当前感知通道能够直接复核的属性，例如颜色、对象类别、支撑关系或邻接语义。若两个实例都命中同一对象概念，系统必须保留全部候选并请求补充属性；收到“拿白色杯子”一类补充后，应重新解释当前观测并记录未选候选的排除理由，而不是把人类回答直接写成目标事实。

“左边那个”或“右边那个”只有在共享参照系已经明确时才允许落地。屏幕左右、观察者左右和执行主体左右不得默认互换；参照系不明时继续澄清。当前样品先验证颜色属性澄清，不以屏幕位置规避参照系问题。
...
当端侧概念或经验不足以继续任务时，执行主体应先输出缺口原因，再进入可选的真人遥操作教学。教学模式只把当前会话中的动作候选生成权临时交给人类，所有控制仍通过当前本体画像、Adapter、碰撞边界、保护策略和 P016 事实验真。教学结束后，不保存人的按键序列或单次轨迹，而把已验真的过程编译为：

1. 目标对象与执行体的类型化绑定槽位；
2. 到达可操作范围、对准和抓取等拓扑关系；
3. 动作方向、本体能力和安全限制；
4. 因果前提、产出事实与销毁事实；
5. 事实终止条件和失败恢复入口。

编译结果先是候选经验，必须由执行主体重新绑定当前对象并自主复做。只有目标物理事实成立且人类确认任务意图得到满足时，才能晋升为本地可信经验；可信经验后续仍需经过稳定复用，才可进入概念晋升候选。

本地可信经验持久化时只保存便携合同：目标概念槽位、目标事实、因果前提与效应、过程不变量、物理约束、终止条件和验证摘要。教学对象实例、教学会话、复做作业和遥操作轨迹不得进入持久化合同。冷启动加载后，端侧概念层必须用当前感知重新绑定目标实例；绑定不唯一或不满足概念槽位时继续澄清，不得沿用旧实例引用。

动作语言必须按物理效应拆分概念算子。例如，“向右转”只改变本体朝向事实，不产生位置位移；“往右边走”对不能横移的轮式本体表示先向右转再沿新前方平移。两者不得因为共享“右”这一方向词而映射为同一执行模板。
