...
| 层 | 关联专利 | 关系 |
|---|---|---|
| L1 事件历史仓库 | **P020** 统一中间表示 | 事件是 RCIR 的核心类型；L1 记录的 Event/Predicate/EvidenceEnvelope 直接使用 P020 定义的数据格式 |
| L2 模式发现引擎 | **P011**/**P012** | 从经验中发现重复模式（对应 P011 后果预测的数据基础）；跨实例一致性分析是多经验归纳的前提（对应 P012 的输入） |
| L3 概念空间 | **P012** 概念归纳跨域迁移 | 概念的组织、距离度量、合并/分裂是 P012 "跨域迁移"的工程基础设施 |
| L4 认识闭环引擎 | **P019** 端侧概念内化 | 运行在端侧环境，但与 P019 现有定位有本质差异（详见 8.2） |
| L4→P018 接口 | **P018** 运行时仲裁 | 安全探针的控制授权 |
| L4→P016 接口 | **P016** 物理验真 | 概念预测的物理验证 |

### 8.2 概念自形成架构 ≠ P019 的简单实现

P019 当前定位（来自 P019 立项书及 P018 分案支撑复核）：

> P019 保护**端侧概念候选如何形成、在当前事实中落地、补齐缺口、受控复用并回退**。

P019 是一个**被动匹配系统**——交互输入来了，才去匹配已有概念，匹配不上就回退。

概念自形成架构的 L4 是一个**主动发现系统**——系统自己监控运行事件流，发现重复模式，自主发起概念形成实验。

关键差异：

| 维度 | P019（当前定位） | 概念自形成 L4 |
|---|---|---|
| **触发方式** | 被动——人类交互输入触发概念匹配 | 主动——L4 持续监控 L2，发现模式即发起 inquiry |
| **概念来源** | 人工配置、教学确认、成功执行记录 | 从运行事件中自主提炼 |
| **是否主动实验** | 否——只匹配和回退 | 是——设计安全探针验证或证伪假设 |
| **是否元学习** | 否——无参数自调 | 是——每次实验后调整判断阈值 |
...
```
 P020 的技术场域
 ┌──────────────────────────────────────────────┐
 │  RCIR 七类基本类型 (Event/Predicate/Concept)   │ ← L1 的数据格式基础
 │  书同文、车同轨、度量衡                        │
 └──────────────────────────────────────────────┘
 ...
 │  概念归纳跨域迁移                              │
 │  多条经验 → 公共概念                           │
 │  L2+L3 是 P012 的自动化工程化实现              │
 │  但 P012 原方案不要求"持续运行、自主驱动"       │
 └──────────────────────────────────────────────┘
```

### 8.4 概念自形成架构对 P019 的支持

P019 尚未定稿（作为 P018 的分案）。当前 P019 独权主链的保守建议为（引自 P019 分案支撑复核）：

> 1. 在自动化执行体端侧维护概念记录…  
> 2. 接收交互输入，并匹配既有概念记录或形成候选概念  
> 3. 读取任务期运行时世界状态快照，将概念所需事实与当前事实对齐  
> 4. 根据已满足/缺失事实生成候选步骤或澄清问题  
> 5. 接收反馈，更新概念的成功确认、复用、衰减或失败回退状态  
> 6. 将概念输出保持为候选，交运行时仲裁决定是否执行  

概念自形成架构可以为 P019 提供以下支撑：

| P019 独权要素 | 概念自形成架构提供的支撑 |
|---|---|
| 步骤 1：端侧维护概念记录 | L3 `ConceptSpace` 提供运行时概念库 + 距离度量 |
| 步骤 2：匹配或形成候选概念 | L4 `EpistemicLoopEngine` 提供"从运行事件自主形成概念候选"的机制——这是 P019 当前未覆盖但可纳入实施例或从属项的"概念自主产生渠道" |
| 步骤 3-4：事实对齐与候选生成 | 当前 P019 工程已有，概念自形成不重复实现 |
| 步骤 5：反馈与状态更新 | L4 元学习层提供更高维度的参数自调（不仅仅是概念复用计数） |
| 步骤 6：候选不取得执行权 | 与当前所有 P 方案的共有边界一致，概念自形成严格继承 |

**核心结论**：概念自形成架构可以作为 P019 说明书中的一个独立实施例或实施方式，解决"概念从哪里来"的问题。P019 独权可以保持"被动匹配"的窄范围（更稳固），而概念自形成作为其从属项或独立分案种子。

### 8.5 从专利角度的定性

| 问题 | 答案 |
|---|---|
| 概念自形成架构是 P020 的实现吗？ | 部分是的——L1 使用 P020 的 RCIR 数据格式。但 P020 的核心是"单次教学→首次可信经验"，不是持续闭环 |
| 概念自形成架构是 P019 的实现吗？ | 不完全是——P019 是被动匹配，概念自形成是主动发现。后者可以作为前者的实施例或分案 |
| 概念自形成架构是 P012 的实现吗？ | 部分是的——L3 概念空间 + 距离度量是 P012 的工程化。但 P012 不要求持续运行和元学习 |
| **概念自形成架构最独特的部分在哪？** | **L4 认识闭环引擎的主动驱动逻辑**——不在任何现有 P 方案中 |
...
P018（已提交）
 └─ P019（分案，待撰写）
      └─ 概念自形成架构（可作为 P019 的从属实施例，
          或在 P019 提交后作为独立分案种子）
```

L4 认识闭环引擎的"主动监控→判断→实验→元学习"循环，是目前整个 P 族中没有被任何方案覆盖的新颖点。它既不是 P019 的"被动匹配"，也不是 P012 的"一次性归纳"，更不是 P020 的"单次教学编译"。
...
阶段 D 的最小真实闭环已经通过：`runtime_epistemic_adapter.py` 将模拟执行器的执行轨迹、事实结果和 P016 多通道证据转换为 L1 记录，同时明确 `current_fact_authority_changed=false`。

三次真实模拟执行共形成 69 条历史记录。L2 从中发现 `p016_fact_outcome` 重复模式，L4 发起认识目标并经 P018/P016 完成验证；概念在字典准入后加入 L3。相同模式到最近概念的距离从 `1.0` 降为 `0.0`，下一轮相同模式被标记为 `already_explained_by_concept=true`，不再重复发起认识目标。

阶段 D 校验入口：`validate_rell_epistemic_flywheel_stage_d.py`。

至此最小飞轮链条已闭合。后续工程重点转为持久化规模、异常/概念漂移、并发 inquiry 调度，以及对真实机器人执行流的长期稳定性测试。
